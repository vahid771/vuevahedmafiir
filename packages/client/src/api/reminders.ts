...
export async function getReminders(token: string): Promise<Reminder[]> {
  const res = await fetch('/api/reminders', { headers: h(token) });
  if (!res.ok) throw new Error('Failed to fetch reminders');
  return res.json();
}

export async function createReminder(token: string, data: Partial<Reminder>): Promise<Reminder> {
  const res = await fetch('/api/reminders', { method: 'POST', headers: h(token), body: JSON.stringify(data) });
  if (!res.ok) throw new Error('Failed to create reminder');
  return res.json();
}

export async function updateReminder(token: string, id: number, data: Partial<Reminder>): Promise<Reminder> {
  const res = await fetch(`/api/reminders/${id}`, { method: 'PATCH', headers: h(token), body: JSON.stringify(data) });
  if (!res.ok) throw new Error('Failed to update reminder');
  return res.json();
}

export async function deleteReminder(token: string, id: number): Promise<void> {
  const res = await fetch(`/api/reminders/${id}`, { method: 'DELETE', headers: h(token) });
